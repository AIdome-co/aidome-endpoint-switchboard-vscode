...
describe('OperationError', () => {
  it('has name "OperationError"', () => {
    const err = new OperationError('internal msg', 'user-friendly msg');
    expect(err.name).toBe('OperationError');
  });

  it('stores the user message', () => {
    const err = new OperationError('plan apply failed', 'Configuration could not be applied');
    expect(err.userMessage).toBe('Configuration could not be applied');
  });

  it('stores optional operation name', () => {
    const err = new OperationError('internal', 'user msg', 'applyPlan');
    expect(err.operation).toBe('applyPlan');
  });

  it('operation is undefined when not provided', () => {
    const err = new OperationError('internal', 'user msg');
    expect(err.operation).toBeUndefined();
  });

  it('is an instance of Error', () => {
    expect(new OperationError('m', 'u')).toBeInstanceOf(Error);
  });

  it('technical message is accessible via .message', () => {
    const err = new OperationError('technical detail', 'friendly msg');
...
describe('withErrorBoundary', () => {
  ...
  it('returns domain outcome for OperationError', async () => {
    const err = new OperationError('apply failed', 'Plan could not be applied', 'applyPlan');
    const outcome = await withErrorBoundary(async () => { throw err; });
    expect(outcome.kind).toBe('domain');
    if (outcome.kind === 'domain') {
      expect(outcome.error).toBe(err);
