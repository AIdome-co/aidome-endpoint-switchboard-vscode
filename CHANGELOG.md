...
- Added format-aware Continue.dev configuration support for YAML and legacy JSONC files, including format-specific path selection, provider-aware model updates, and regression coverage.
- Added a safe on-disk VS Code extension detection fallback for remote/server hosts where `vscode.extensions.all` does not expose an installed extension.
