...
- Hardened the Kilo Code Tier B adapter against upstream config behavior: use native `provider.<id>` JSONC, honor `KILO_CONFIG_DIR`/`KILO_CONFIG`, authenticate model discovery from SecretStorage without serializing tokens, preserve existing provider models/auth/settings, and structurally verify JSONC plus selectable models while guiding runtime authentication when required.
...
- Expanded `kiloConfigPatcher` regression coverage to 100% branch/stmt/func/line: malformed provider entries (non-record `options`/`models`, blank model ids), auth-reference detection negatives, and `mergeModelEntries` duplicate/overlap handling confirm upstream JSONC shape is preserved on apply.
