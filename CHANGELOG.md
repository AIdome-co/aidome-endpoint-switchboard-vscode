...
- Hardened the maintenance controller's environment-vs-regression handling: when the deterministic 100% gate already certifies a PR head but supplementary local validation fails *only* for an environment deferral (headless/memory E2E, `returncode=None`), the controller now records an honest environment deferral and skips the wasted Hermes cycle (which cannot fix the host), keeping the PR below 100% (fail-closed) until a later run has headroom — instead of burning a cycle and misreporting a source-code regression. Reconcile now mirrors an already-eligible live gate as `eligible100` in durable state. Adds regression tests for the env-deferral classifier, the no-cycle skip, and real-failure preservation.
- Anchored the maintenance digest PR count to the LIVE in-scope inventory instead of the subset recorded per run, so partial / reconcile-only / budget-paused runs no longer report a misleading "0 PRs in scope" while the inventory still contains the PRs. A genuinely empty live inventory still reports 0 (it no longer falls back to stale persisted PRs). Adds regression tests for inventory-anchored counts and the truly-empty case.
- Made the E2E xvfb validation tests deterministic: they now force a non-critical memory-pressure gate so the xvfb wrap is isolated from host memory state (previously they silently deferred E2E and failed on a memory-starved host).
