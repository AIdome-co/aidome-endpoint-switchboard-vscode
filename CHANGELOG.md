...
- Hardened the Kilo Code Tier A adapter against upstream config behavior: use native `provider.<id>` JSONC, honor `KILO_CONFIG_DIR`/`KILO_CONFIG`, authenticate model discovery from SecretStorage without serializing tokens, preserve existing provider models/auth/settings, and structurally verify JSONC plus selectable models.
