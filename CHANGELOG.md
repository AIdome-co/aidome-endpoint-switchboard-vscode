...
- Hardened Continue.dev config.json routing to match upstream JSONC parsing and model validation, preserve existing settings, update only an existing OpenAI model, add one valid model when absent, and support a missing config file.
