...
- Fixed OpenAI Codex routing to use the current `model_providers` TOML schema, preserve existing models/providers, reject malformed config safely, and verify the selected Responses provider instead of accepting any provider table.
- Aligned the GA-readiness doc's Codex routing row with the `model_providers` schema and added Codex adapter/patcher regression tests for malformed roots, non-table provider entries, missing provider entries, invalid names, and invalid environment-var names.
