...
export async function assignProfileAssistants(
  ...
  for (const assistantKey of deselectedAssistantKeys) {
    try {
      await profileStore.deleteAssistantMapping(assistantKey);
...
function buildAssistantSelectionItems(
  ...
  for (const assistantKey of candidateKeys) {
    ...
    const mapping = mappings.find(item => item.assistantKey === assistantKey);
    const currentProfile = mapping
      ? profiles.find(item => item.id === mapping.profileId)
      : undefined;
    ...
    items.push({
      ...
      detail: currentProfile
        ? currentProfile.id === profile.id
          ...
          : `${detectionLabel} · currently assigned to ${currentProfile.name}`
        : detectionLabel,
      picked: currentProfile?.id === profile.id
    });
  }
